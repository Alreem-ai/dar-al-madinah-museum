...
const temp_chunk_1 = loadChunk('temp_chunk_1.json'); // friday-prayer, year-of-elephant (partial)
const temp_chunk_2 = loadChunk('temp_chunk_2.json'); // ansar-love, yathrib-inhabitants, kaaba-quraysh (partial)
const temp_chunk_3 = loadChunk('temp_chunk_3.json'); // valley-ibrahim-hill, kaaba-ottoman, mosque-qibla-change (partial)
...
const chunk_15_16 = loadChunk('chunk_15_16.json');
const temp_chunk_4 = loadChunk('temp_chunk_4.json');

// Base 1-4 from finalize_restoration.js context (hardcoded for safety)
const artifacts_1_4 = [
    {
      "id": "arrival-model",
      ...
      "description": { 
        ...
        "ar": "تابع النبي وصاحبه أبي بكر الصديق الطريق إلى المدينة بصحبة خادمه عامر بن فهيرة ودليلهما الذي سلك بهما الطريق إلى أسفل مكة ثم مضى بهما على الساحل حتى وصل الركب إلى ثنية العائر عن يمين ركوبه، وهبط الركب إلى بطن الوادي ثم اعتلى الحرةليهبط إلى المنخفض ويسير محاذياً لـ وادي الرانوناء حتى وصل قباء.",
...
const al_qibli_mosque = {
    ...
    "material": { "en": "Diorama Model", "ar": "مجسم ديوراما", "fr": "Modèle de Diorama", "ur": "ڈیوراما ماڈل", "id": "Model Diorama" },
    "origin": { "en": "Jerusalem, Dar Al-Madinah Museum", "ar": "القدس، متحف دار المدينة", "fr": "Jérusalem, Musée Dar Al-Madinah", "ur": "يروشلم، دار المدينة ميوزيم", "id": "Yerusalem, Museum Dar Al-Madinah" },
    "image": "/images/artifacts/al-qibli-mosque.png",
    "audioUrl": null
};

// 14: kaaba-zubayr (from chunk_12_14 or temp_chunk_4)
const kaaba_zubayr = temp_chunk_4.find(a => a.id === 'kaaba-zubayr');

// 15: quranic-writing-materials (from chunk_15_16)
const quran_materials_raw = chunk_15_16.find(a => a.id === 'quranic-writing-materials');
// Add missing gallery images for Model 15
const quran_materials = {
    ...quran_materials_raw,
    "images": [
        "/images/artifacts/quran-materials-1.png",
        "/images/artifacts/quran-materials-2.png",
        "/images/artifacts/quran-materials-3.png"
    ]
};

const final_artifacts = [
    ...artifacts_1_4,
    temp_chunk_1[0], // 5: friday-prayer
    revelation_hira, // 6: revelation-hira
    temp_chunk_1[1], // 7: year-of-elephant
    temp_chunk_2[0], // 8: ansar-love
    temp_chunk_2[1], // 9: yathrib-inhabitants
    temp_chunk_2[2], // 10: kaaba-quraysh
    temp_chunk_3[0], // 11: valley-ibrahim-hill
    temp_chunk_3[1], // 12: kaaba-ottoman
    al_qibli_mosque, // 13: al-qibli-mosque
    kaaba_zubayr,    // 14: kaaba-zubayr
    quran_materials  // 15: quranic-writing-materials
];

// Verify IDs and Titles
final_artifacts.forEach((a, i) => {
    console.log(`${i+1}: ${a.id} - ${a.title.en}`);
});
...
console.log("Successfully restored 15 artifacts to data.json");
